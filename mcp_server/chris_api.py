...
# Base URL for ChRIS API
CHRIS_URL = "http://localhost:8000/api/v1"

# Function to get the list of plugins from ChRIS
def get_plugins(username: str, password: str) -> dict:
    url = f"{CHRIS_URL}/plugins/"
...
def get_plugin_instance_details(username: str, password: str, instance_id: int) -> dict:
    url = f"{CHRIS_URL}/plugins/instances/{instance_id}/"
    response = requests.get(url, auth=(username, password))
...
def get_pacs_files(username: str, password: str) -> dict:
    url = f"{CHRIS_URL}/pacs/files/"
    response = requests.get(url, auth=(username, password))
    response.raise_for_status()
    return response.json()

# Function to get the list of user files
def get_user_files(username: str, password: str) -> dict:
    url = f"{CHRIS_URL}/userfiles/"
    response = requests.get(url, auth=(username, password))
    response.raise_for_status()
    return response.json()

# Function to get a list of pipelines
def get_pipelines(username: str, password: str) -> dict:
    url = f"{CHRIS_URL}/pipelines/"
    response = requests.get(url, auth=(username, password))
    response.raise_for_status()
    return response.json()

# Function to get details of a specific pipeline by its ID
def get_pipeline_details(username: str, password: str, pipeline_id: int) -> dict:
    url = f"{CHRIS_URL}/pipelines/{pipeline_id}/"
    response = requests.get(url, auth=(username, password))
    response.raise_for_status()
    return response.json()

# Function to search for a specific plugin or resource
def search_plugins(username: str, password: str, query: dict) -> dict:
    url = f"{CHRIS_URL}/plugins/search/"
    response = requests.get(url, params=query, auth=(username, password))
    response.raise_for_status()
    return response.json()

# Function to create a new pipeline
def create_pipeline(username: str, password: str, pipeline_data: dict) -> dict:
