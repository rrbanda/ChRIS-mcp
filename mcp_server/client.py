...
async def main(server_url: str, article_url: str = None):
    """Connect to the MCP server, list its capabilities, and call tools if arguments provided.
    ...
    try:
        async with sse_client(server_url) as streams:
            async with ClientSession(streams[0], streams[1]) as session:
                ...
                # Optionally call read_wikipedia_article
                if article_url:
                ...
                # Always call get_chris_root with explicit URL
                print("\n🔍 Calling get_chris_root tool...")
                try:
                    response = await session.call_tool(
    "get_chris_root", arguments={"args": {"url": "https://cube.chrisproject.org/api/v1/"}}
)

                    print("\n=== 🧠 ChRIS API Root Response ===\n")
                    print(response)
                except Exception as chris_exc:
                    print("❌ Error calling get_chris_root tool:")
                    traceback.print_exception(
                        type(chris_exc), chris_exc, chris_exc.__traceback__
                    )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Usage: python client.py <server_url> [<wikipedia_article_url>]\n"
            "Example: python client.py http://localhost:8000/sse https://en.wikipedia.org/wiki/Python_(programming_language)"
        )
        sys.exit(1)
